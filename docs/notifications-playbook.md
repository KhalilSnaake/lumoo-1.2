...
- **Icône de notif Android (barre d'état)** = `expo-notifications.icon` (+ `color`) dans `app.json`. ⚠️ Elle doit être **monochrome (blanc sur transparent)** : Android n'affiche que la **silhouette** (alpha), peinte en blanc puis teintée par `color`. Une icône **couleur** (ex. l'icône d'app) ressort en **carré/blob blanc**. → fournir un PNG blanc-sur-transparent dédié (~96×96+). Changement **natif → rebuild** (pas d'OTA). **iOS** : pas d'icône custom (affiche l'icône d'app). ✅ Lumoo : `apps/mobile/assets/images/notification-icon.png` (panier blanc/transparent, 512×512, remplit ~82 % du cadre) — **ne pas** repointer sur `icon.png` (opaque → carré blanc).
